...
// OrderItemInput mirrors jebbs-dashboard's lib/hooks/orders/use-create-order.ts
// (OrderItemInput, :7-23) -- that hook does not exist in this repo (order
// submission here is a server route, built in WU4), so the wire shape lives
// alongside its producer (OrderDataTransformer) instead. WU4's
// lib/order/create-web-order.ts imports it from here.
...
export class OrderDataTransformer {
  ...
  // Sides as independent order items
  static transformSidesToOrderItems(sides: SelectedSide[]): OrderItemInput[] {
    return sides.map((s) => ({
      ...
      // Only the base price -- extras go separately, see below
